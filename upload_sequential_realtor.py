#!/usr/bin/env python3
"""
Upload videos sequentially from a folder – REALTOR CHANNEL.
Supports repost videos with custom credits.
Videos must be named with a number (e.g., 1.mp4, 2.mp4, 001.avi, 10.mov).
Credits are defined in credits_realtor.json (mapping number -> credit text).
State is saved in video_sequence_state_realtor.json and committed to the repo.
"""
...
VIDEO_FOLDER = "video_of_day_realtor"
STATE_FILE = "video_sequence_state_realtor.json"
CREDITS_FILE = "credits_realtor.json"
...
def load_credits():
    """Load credits mapping from credits_realtor.json (if exists). Returns dict number->credit_string."""
    if os.path.exists(CREDITS_FILE):
        try:
            with open(CREDITS_FILE, "r") as f:
                return json.load(f)
        except json.JSONDecodeError:
            print("⚠️ credits_realtor.json is invalid, using empty dict")
            return {}
    return {}
...
def commit_and_push_state():
    ...
    try:
        ...
        subprocess.run(["git", "add", STATE_FILE, CREDITS_FILE], check=True)
...
def build_description(number, credit_dict):
    ...
    # Footer with real estate hashtags
    base_desc += "\n\n🔗 stupidorange.com\n#stupidorange #creativedaily #VideoOfTheDay #DubaiRealEstate #PropertyInvestment #realestateagent #UAE"
    return base_desc
